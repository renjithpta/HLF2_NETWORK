...
if [ "$CC_SRC_LANGUAGE" = "go" -o "$CC_SRC_LANGUAGE" = "golang" ] ; then
	...
	#CC_SRC_PATH="./chaincode/java/agreements/build/install/agreements"
  CC_SRC_PATH="./chaincode/"

	echo Compiling Java code ...
	#pushd ./chaincode/java/agreements
#	./gradlew installDist
	#popd
...
chaincodeInvokeInit() {
  ...
  # it using the "-o" option
  set -x
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CHAINCODE_NAME} $PEER_CONN_PARMS --isInit -c '{"function":"setOptions","Args":[ "ARBToken","ARBT","2"]}' >&log.txt
  res=$?
  set +x
  cat log.txt
...
chaincodeInvokeMint() {
  ...
  # it using the "-o" option
  set -x
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CHAINCODE_NAME} $PEER_CONN_PARMS  -c '{"function":"mint","Args":["100000000000"]}' >&log.txt
  res=$?
  set +x
  cat log.txt
  verifyResult $res "Invoke execution on $PEERS failed "
...
chaincodeInvokeTransfer() {
  ...
  # it using the "-o" option
  set -x
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CHAINCODE_NAME} $PEER_CONN_PARMS  -c '{"function":"transfer","Args":["x509::CN=Admin@org2.example.com, OU=admin, L=San Francisco, ST=California, C=US::CN=ca.org2.example.com, O=org2.example.com, L=San Francisco, ST=California, C=US","100000000"]}' >&log.txt
  res=$?
  set +x
  cat log.txt
  verifyResult $res "Invoke execution on $PEERS failed "
...
chaincodetoTalSupplyQuery() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n ${CHAINCODE_NAME} -c '{"Args":["totalSupply"]}' >&log.txt
    res=$?
    set +x
     set -x
    peer chaincode query -C $CHANNEL_NAME -n ${CHAINCODE_NAME} -c '{"Args":["balanceOf","x509::CN=Admin@org1.example.com, OU=admin, L=San Francisco, ST=California, C=US::CN=ca.org1.example.com, O=org2.example.com, L=San Francisco, ST=California, C=US"]}' >&log3.txt
    res=$?
    set +x
    setGlobals 2
    set -x
    peer chaincode query -C $CHANNEL_NAME -n ${CHAINCODE_NAME} -c '{"Args":["totalSupply"]}' >&log2.txt
    res=$?
    set +x

    set -x
    peer chaincode query -C $CHANNEL_NAME -n ${CHAINCODE_NAME} -c '{"Args":["balanceOf","x509::CN=Admin@org2.example.com, OU=admin, L=San Francisco, ST=California, C=US::CN=ca.org2.example.com, O=org2.example.com, L=San Francisco, ST=California, C=US"]}' >&log4.txt
    res=$?
    set +x

    set -x
    peer chaincode query -C $CHANNEL_NAME -n ${CHAINCODE_NAME} -c '{"Args":["getClientAccountID"]}' >&log1.txt
    res=$?
    set +x
		let rc=$res
		COUNTER=$(expr $COUNTER + 1)
  ...
  cat log.txt
  echo ""
   echo "Id of org2 client"
  cat log1.txt
  echo "Balance Of org1 "
  cat log3.txt
   echo "Balance Of org2"
  cat log4.txt
...
chaincodeQuery() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    echo "Attempting to Query peer0.org${ORG} ...$(($(date +%s) - starttime)) secs"
    set -x
    peer chaincode query -C $CHANNEL_NAME -n ${CHAINCODE_NAME} -c '{"Args":["tokenName"]}' >&log.txt
    res=$?
    set +x
		let rc=$res
		COUNTER=$(expr $COUNTER + 1)
...
## Invoke the mint
chaincodeInvokeMint 1 2

sleep 10

# Transfer from org1 peer0 to org2 peer0
chaincodeInvokeTransfer 1 2

## Query the  total supply and clientid
chaincodetoTalSupplyQuery 1
